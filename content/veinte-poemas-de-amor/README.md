...
1. **`code/languages.py`** — Google Translate's 2016 language roster (sorted by
   ISO code), the cyclic `chain()` that reproduces the succession, and English
   names for each code.
...
4. **`code/build_gallery.py`** — assembles the self-contained
   `results/despoetitzacio.html`: Space Mono on a cream ground, a sticky index,
   and per poem a sweep through the 104 languages that dims the original and
   reveals the despoetized residue. Everything is inlined (no external requests).
...
python code/build_gallery.py         # rebuild results/despoetitzacio.html
```
...
- `results/despoetitzacio.html` — the built gallery (served verbatim as the
...
- `code/` — `languages.py`, `despoetize.py`, `main.py`, `build_gallery.py`.
