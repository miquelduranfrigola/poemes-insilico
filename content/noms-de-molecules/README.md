...
`code/build_web.py` → `results/index.html`: a self-contained page that reads as a
**standalone poem** in the collection's visual language (Space Mono on a cream
ground — same tokens and vendored webfont as `poemes-proteics`). The poem runs down
...
Eventually to be merged with `poemes-proteics` into one shared gallery.
...
conda run -n <env> python code/build_web.py           # results/index.html (RDKit + Node)
```
